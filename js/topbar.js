...
const TopbarModule = (function() {
    ...
    // ========== ПЕРЕКЛЮЧЕНИЕ РАЗДЕЛОВ (исправленное) ==========
    function switchToSection(section) {
        ...
        // Переключение основных контейнеров (кроме проектов)
        ...
        if (section === 'projects') {
            ...
            if (detailContainer) {
                ...
                detailContainer.innerHTML = ''; // Очищаем содержимое!
            }
            // Вызываем hideDetail на случай, если детальная страница была открыта
            if (typeof ProjectDetail !== 'undefined' && ProjectDetail.hideDetail) {
                ProjectDetail.hideDetail();
            }
        }

        // Рендеринг содержимого других разделов
